...
class ProductRepositorySpecs extends Specification {
    ...
    def "ProductRepository.find() should return collection of Products filtered by name"() {
        setup:
        def filter = [name: "Kamera"]

        when:
        Collection<Product> products = productRepository.find(filter)

        then:
        products.each { it.name.contains("Kamera".toLowerCase())}
        products.size() != 0
    }
